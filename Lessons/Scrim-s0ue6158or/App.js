...
*/
	...
	const pixelElements = pixels.map((pixel) => (
		<div
            ...
            onMouseEnter={() => handleDraw(pixel.id)}
		></div>
	))
    ...
    function handleDraw(id){ 
        if(mouseDown && wantsToDraw){
             const pixelsEdited = pixels.map(pixel => {
                return pixel.id === id ? {...pixel, filled: true} : pixel
            })
            setPixels(pixelsEdited)
        } else{
            const pixelsEdited = pixels.map(pixel => {
                return pixel.id === id ? {...pixel, filled: false} : pixel
            })
            setPixels(pixelsEdited)
        }
    }
