...
function isAnagramInArray(anagram, arr){
    const anagramFormatted = reformatAnagram(anagram)
    const compareAnagram = arr.filter(item => {
        const itemFormatted = reformatAnagram(item)
        if(anagramFormatted === itemFormatted){
            return item
        }
    })
    return compareAnagram.length && "Anagram is in the array"
}

function reformatAnagram(anagram){
    const removeAnagramSpace = anagram.replaceAll(" ", "").toLowerCase().split("")
    const reformatAnagram = removeAnagramSpace.sort().join("")
    return reformatAnagram
}
